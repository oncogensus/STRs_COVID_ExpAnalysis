...
## To df_strs_outliers: clean gene_id
...
## ==================================================================
## DEFINE BACKGROUND AND FOREGROUND FOR KEGG ENRICHMENT
## ==================================================================

## Background: ALL genes expressed in the lung
...
## KEGG enrichment with Entrez IDs
enrich_result <- enrichKEGG(
  gene         = case_gene_list,
  organism     = "hsa",              # hsa = Homo sapiens
  keyType      = "ncbi-geneid",      # KEGG uses NCBI Gene IDs (Entrez)
...
if (!is.null(enrich_result) && nrow(enrich_result@result) > 0) {
  ...
  # Save RDS object
  saveRDS(enrich_result, file = "results/enrich_resultsKEGG.rds")
  
  ## Transform results into data frame and save CSV
  dat.results <- data.frame(enrich_result, stringsAsFactors = FALSE)
  write.csv(dat.results, file = "results/KEGGenrich_p-value0.05_allSTRs.csv", row.names = FALSE)
  
  cat("\n✅ KEGG enrichment completed successfully!\n")
  cat("Number of enriched pathways:", nrow(dat.results), "\n")
  
} else {
  cat("\n⚠️ WARNING: No significantly enriched KEGG pathways found.\n")
