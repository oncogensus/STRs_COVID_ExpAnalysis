...
#
# Depende de data.table e dbscan instalados no env micromamba `r_env`:
#   micromamba install -n r_env -c conda-forge -y r-data.table r-dbscan
#
# Uso:
#   Rscript run_dbscan_subset.R <input_residuals.tsv> <output.tsv>
for (pkg in c("data.table", "dbscan")) {
  if (!requireNamespace(pkg, quietly = TRUE)) {
    stop("Pacote '", pkg, "' ausente no env `r_env`. Instale com:\n",
         "  micromamba install -n r_env -c conda-forge -y r-", pkg, "\n")
  }
}
library(data.table)
library(dbscan)
