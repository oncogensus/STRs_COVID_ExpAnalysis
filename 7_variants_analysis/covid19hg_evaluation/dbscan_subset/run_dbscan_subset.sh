...
# Garante data.table/dbscan no ambiente micromamba `dbscan` (binarios conda-forge)
micromamba install -n dbscan -c conda-forge -y r-data.table r-dbscan 2>&1 | tail -5 || \
  echo "AVISO: nao foi possivel instalar via micromamba; se os pacotes ja existirem no env dbscan, pode ignorar."
...
# R roda no env micromamba `dbscan` (tem data.table/dbscan)
micromamba run -n dbscan Rscript run_dbscan_subset.R data/suggestive_strs_residuals.tsv results/suggestive_strs_outliers.tsv
